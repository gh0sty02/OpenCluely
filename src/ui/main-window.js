...
class MainWindowUI {
    ...
    if (!this.statusDot || !this.skillIndicator || !this.micButton || !this.screenshotButton) {
        ...
        if (this.languageSelect) {
            // Set default to JavaScript if no value is set
            ...
            if (window.electronAPI && window.electronAPI.getSettings) {
                window.electronAPI.getSettings().then(settings => {
                    if (settings && settings.codingLanguage) {
                        ...
                        // Save JavaScript as default if no language is set
                        this.languageSelect.value = 'javascript';
                        window.electronAPI.saveSettings({ codingLanguage: 'javascript' });
                    }
                }).catch(() => {
                    // Fallback to JavaScript on error
                    this.languageSelect.value = 'javascript';
                });
            }
